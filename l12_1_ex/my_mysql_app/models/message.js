...
async function update(message, newTxt) {
    try {
        await db.pool.beginTransaction(); 
        const [rows, fieldDefs] = await db.pool.query(`
            SELECT * FROM ${tableName} where time = ? AND msg = ?`, [message.time, message.msg]
        );
        if (rows.length > 0) {
            await db.pool.execute(`
                UPDATE ${tableName} SET time = ? AND msg = ?`, [message.time, message.msg + newTxt]
            );
        }
...
module.exports =  { Message, all, sync, insertOne, insertMany, deleteOne, update }